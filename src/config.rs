...
// 固定服务器配置 - 飞剑 RustDesk
pub const RENDEZVOUS_SERVERS: &[&str] = &["fjrust1.ensp.top"];
pub const RS_PUB_KEY: &str = "pk3Nlnlehmpypn2cqj61iNWhSKGTzyo2y65rUiVvz7E=";
...
impl Config {
    ...
    pub fn get_rendezvous_server() -> String {
        // 飞剑 RustDesk：优先使用用户选择的线路（存储在 custom-rendezvous-server 中）
        // 如果没有配置，则使用默认的固定服务器
        let mut rendezvous_server = Self::get_option("custom-rendezvous-server");
        
        // 如果用户没有选择线路，使用默认的第一条线路
        if rendezvous_server.is_empty() {
            rendezvous_server = RENDEZVOUS_SERVERS
                .first()
                .map(|s| s.to_string())
                .unwrap_or_else(|| "fjrust1.ensp.top".to_string());
            // 自动设置默认服务器配置
            Self::set_option("custom-rendezvous-server".to_string(), rendezvous_server.clone());
            Self::set_option("relay-server".to_string(), rendezvous_server.clone());
            Self::set_option("api-server".to_string(), rendezvous_server.clone());
            Self::set_option("key".to_string(), RS_PUB_KEY.to_string());
        }
